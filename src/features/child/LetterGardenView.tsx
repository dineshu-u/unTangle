...
export const LetterGardenView: React.FC = () => {
  ...
  const handleTeachMe = (creature: LetterItem) => {
    ...
    sounds.speak(`${creature.letter} ... ${creature.creatureName} ... ${creature.meaning}`, language);
  ...
  const speakMatchTarget = () => {
    ...
    if (language === 'ta') {
      sounds.speak(`எந்த படம் '${currentTarget.letter}' எழுத்தில் தொடங்கும்?`, 'ta');
    } else {
      sounds.speak(`Which item starts with letter '${currentTarget.letter}'?`, 'en');
    }
  };
