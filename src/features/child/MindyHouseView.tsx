...
export const MindyHouseView: React.FC = () => {
  ...
  const handleSelectOption = async (optId: string, isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      if (language === 'ta') {
        sounds.speak(`நன்றி! ${currentScenario.targetWord} என்றால் என்னவென்று எனக்கு அருமையாக புரிந்தது!`, 'ta');
      } else {
        sounds.speak(`Thank you! Now I know what ${currentScenario.targetWord} really means!`, 'en');
      }
      ...
      if (language === 'ta') {
        sounds.speak('ம்ம்ம்... இது சரியான விடை இல்லை!', 'ta');
      } else {
        sounds.speak("Hmm... that's not quite right!", 'en');
      }
  ...
  const speakPrompt = () => {
    ...
    sounds.speak(`${currentScenario.sillyClaim} ${currentScenario.prompt}`, language);
  };
