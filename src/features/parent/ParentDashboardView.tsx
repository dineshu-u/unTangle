          ...
          {activeTab === 'settings' && (
            <div className="space-y-3">
              <div className="bg-white rounded-2xl p-3.5 border border-stone-200 shadow-2xs space-y-2.5">
                ...
                {/* Dyslexia-Friendly Font Toggle (UntangleTamil & OpenDyslexic) */}
                <div className="flex items-center justify-between py-1.5 border-b border-stone-100">
                  <div>
                    <span className="text-xs font-bold text-stone-800 block">
                      {language === 'ta' ? 'டிஸ்லெக்ஸியா சிறப்பு எழுத்துரு (UntangleTamil)' : 'Dyslexia-Friendly Typeface'}
                    </span>
                    <span className="text-[10px] text-stone-500">
                      {language === 'ta'
                        ? (useDyslexicFont ? 'UntangleTamil இயக்கத்தில் உள்ளது (அணைத்தால் நோட்டோ சான்ஸ் தமிழ்)' : 'நோட்டோ சான்ஸ் தமிழ் இயக்கத்தில் உள்ளது')
                        : (useDyslexicFont ? 'UntangleTamil (TA) & OpenDyslexic (EN) active. Turn off for standard Noto Sans Tamil.' : 'Standard fonts active (Noto Sans Tamil / System).')}
                    </span>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    title={useDyslexicFont ? 'Dyslexic Font Enabled' : 'Standard Font Enabled'}
                  >
                    <div
