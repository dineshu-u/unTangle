    ...
    <div className="w-full flex-1 flex flex-col relative overflow-y-auto overflow-x-hidden">
      ...
      {viewMode === 'map' && (
        <div className="relative w-full flex-1 min-h-0 h-full bg-[#97c75c] overflow-hidden select-none">
          ...
          {/* Gentle ambient storybook life: peaceful cloud and fluttering butterfly (not overwhelming) */}
          <div className="absolute top-4 left-8 opacity-75 pointer-events-none">
            <span className="text-3xl drop-shadow-sm">☁️</span>
          </div>
          <div className="absolute top-16 right-20 pointer-events-none z-10 butterfly-flight">
            <span className="text-2xl drop-shadow-sm">🦋</span>
          </div>

          {/* Interactive Storybook Landmark Pins (Calm idle, playful spring bounce on hover/tap!) */}
          {locations.map((loc) => {
            const isHovered = hoveredLocation === loc.id;
            const recommendedScreenId: ScreenId = 
              adaptiveRecommendation?.recommendedActivity === 'teach_mindy'
                ? 'mindy_house'
                : (adaptiveRecommendation?.recommendedActivity as ScreenId) || 'mindy_house';
            const isRecommended = loc.id === recommendedScreenId;
            ...
            return (
              <div
                ...
                className={`absolute z-10 -translate-x-1/2 -translate-y-1/2 group ${
                  isRecommended ? 'pin-recommended' : ''
                }`}
                onMouseEnter={() => setHoveredLocation(loc.id)}
                onMouseLeave={() => setHoveredLocation(null)}
              >
                {/* Single Recommended Adventure Guide (Points kid gently to next goal) */}
                {isRecommended && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-amber-400 to-orange-400 text-white text-[8.5px] font-black px-2 py-0.5 rounded-full border border-white shadow-md flex items-center gap-0.5 whitespace-nowrap z-20 pointer-events-none">
                    <span>⭐</span>
                    <span>{language === 'ta' ? 'அடுத்த ஆட்டம்!' : 'Play Next!'}</span>
                  </div>
                )}

                {/* Tactile Storybook Pin Button: Calm at rest, springy & playful on interaction */}
                <button
                  onClick={() => handleNavigate(loc.id)}
                  className="relative flex flex-col items-center justify-center cursor-pointer focus:outline-none group transition-transform duration-200 ease-out group-hover:scale-115 group-hover:-translate-y-1 active:scale-95"
                  title={title}
                >
                  {/* Warm Sunny Halo — ONLY appears on hover / focus */}
                  <span className="absolute -inset-2 rounded-full bg-amber-300/60 blur-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none" />

                  {/* Playful Star Sparkle — ONLY appears on hover */}
                  <span className="absolute -top-2 -right-1 text-xs select-none pointer-events-none opacity-0 group-hover:opacity-100 group-hover:scale-125 transition-all duration-200">
                    ✨
                  </span>

                  {/* 3D Storybook Landmark Badge */}
                  <div
                    className={`w-11 h-11 sm:w-13 sm:h-13 rounded-full bg-gradient-to-tr ${loc.bgColor} text-white shadow-lg shadow-emerald-950/25 border-2 border-white flex items-center justify-center text-xl sm:text-2xl transition-transform duration-200 group-hover:rotate-6`}
                  >
                  ...
                  {/* Parchment Title Tag */}
                  <div className="mt-1 bg-white/95 backdrop-blur-xs border border-amber-300/80 rounded-full px-2 py-0.5 shadow-xs flex items-center gap-1 transition-all group-hover:bg-amber-100 group-hover:border-amber-400">
